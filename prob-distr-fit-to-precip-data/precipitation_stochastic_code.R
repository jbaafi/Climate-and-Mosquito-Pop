# -------------------------------------------------------------------
# Title: Fitting probability density function to precipitation data
# Author: Joseph Baafi
# Date: 15-09-2021
# -------------------------------------------------------------------
...
plot(density(precip$Total.Precip),main="Density estimate of data")
...
# We fit a model (PDF) to the cleaned data
...
# # Plot the log-normal distribution as above with the estimated parameters
# mu <- 0.29513
# delta <- 1.87584 
# ln <- (1/(x*delta*sqrt(2*pi)))*exp(-((log(x)-mu)^2)/(2*delta^2))
# lines(x, ln, col = "green")
...
# Random sampling
sample(dist.df$precip.val, 1, replace = T, prob=dist.df$lnorm.pdf)
