...
setwd("/Users/jbaafi/Documents/GitHub/climate-and-mosquitoes/mosq-pop-prediction-model")
...
climate_11 <- read.csv("en_climate_daily_ON_6153301_2011_P1D.csv", header = TRUE)
climate_12 <- read.csv("en_climate_daily_ON_6153301_2012_P1D.csv", header = TRUE)
climate_13 <- read.csv("en_climate_daily_ON_6153301_2013_P1D.csv", header = TRUE)
climate_14 <- read.csv("en_climate_daily_ON_6153301_2014_P1D.csv", header = TRUE)
climate_15 <- read.csv("en_climate_daily_ON_6153301_2015_P1D.csv", header = TRUE)
climate_16 <- read.csv("en_climate_daily_ON_6153301_2016_P1D.csv", header = TRUE)
...
# Remarks (04/07/2022) Precip data does not have a trend in it so a periodic
# function can not produce/represent the data. The best way to handle this is to 
# treat it as a random event and fit a probability distribution to the data which has
# been done in another file. 
...
# average temperature data set. The function is cas(x) = sin(x) + cos(x). 
# I have to find a probability density function to estimate the rainfall data set. 
################################################################################
...
# Fit scalar in genotrophic cycle rate function
alpha <- 168.0257252
beta <- 34.0632871
gamma <- 0.4371833 
